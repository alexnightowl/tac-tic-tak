...
export default function LoginPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    try {
      const r = await http.post<{ token: string; user: { id: string; nickname: string } }>('/auth/login', { nickname, password });
      setToken(r.token);
      setUser(r.user);
  ...
  return (
    <main className="min-h-dvh flex items-center justify-center px-6">
      <form onSubmit={submit} className="w-full max-w-sm space-y-5 glass rounded-3xl p-7">
        ...
        <Button type="submit" className="w-full" size="lg" disabled={loading}>{loading ? 'Logging in…' : 'Log in'}</Button>
